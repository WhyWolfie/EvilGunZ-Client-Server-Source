...
#define STR_GUNZ_PROCESS_NAME				"EvilGunZ"
#define STR_GUNZ_EXCUTE_FILENAME			"EvilGunZ.exe"
#define STR_LAUNCHER_EXCUTE_FILENAME		"EvilLauncher.exe"
#define STR_PATCHINFO_FILENAME				"patch.xml"
#define STR_WEBPAGE_URL						"http://evilgunz.sytes.net/start.html" //Your launcher start page url.
#define STR_ADDRESS_REAL					"evilgunz.sytes.net" //Your ip address here
#define STR_ADDRESS_ALPHA					"evilgunz.sytes.net" //Your ip address here
#define STR_DIRECTORY_REAL					"EGunZUpdate" //Your path for yout patch
#define STR_DIRECTORY_ALPHA					"EGunZUpdate" //Your path for yout patch test
#endif LOCALE_NHNUSA
...
void CNewAppDlg::GetRunInfo()
{
	...
	if ( file.Open( "GunzLauncher.ini", CFile::modeRead, &ex) == TRUE)
	{
		...
		// 업데이트를 하지 않는지 검사
	if ( strstr( szBuff, "@SKIPEG_UPDATE") != NULL)
			m_bSkipUpdate = true;

		// Alpha 모드인지 검사
		if ( strstr( szBuff, "@TESTEG_LAUNCH") != NULL)
			m_bIsAlpha = true;
