...
void ZMyCharacter::OnShotRange()
{
	...
	if ((pSelectedItem->GetDesc()->m_nWeaponType.Ref() == MWT_SNIFER))
	{
		return;
	}
	...
	if( ( wtype == MWT_SHOTGUN ) || ( wtype == MWT_SAWED_SHOTGUN ) || ( wtype == MWT_SHOTGUN_RARE ) || ( wtype == MWT_SAWED_SHOTGUN_RARE ) || ( wtype == MWT_SHOTGUN_EXOTIC ) || ( wtype == MWT_SAWED_SHOTGUN_EXOTIC ))
	{
		int sel_type = GetItems()->GetSelectedWeaponParts();
...
void ZMyCharacter::OnShotItem()
{
	...
	// 바닥에 떨구는 아이템인 경우
	else if( nWeaponType==MWT_MED_KIT || nWeaponType==MWT_MED_KIT_RARE || nWeaponType==MWT_MED_KIT_EXOTIC || nWeaponType==MWT_REPAIR_KIT ||	nWeaponType==MWT_REPAIR_KIT_RARE ||nWeaponType==MWT_REPAIR_KIT_EXOTIC || nWeaponType==MWT_BULLET_KIT ||	nWeaponType==MWT_BULLET_KIT_RARE ||	nWeaponType==MWT_BULLET_KIT_EXOTIC ||	nWeaponType==MWT_FOOD ||	nWeaponType==MWT_FOOD_RARE ||	nWeaponType==MWT_FOOD_EXOTIC )
	{
...
void ZMyCharacter::OnGadget_Wfgun()
{
	ZMyCharaterStatusBitPacking & zStatus = m_statusFlags.Ref();
...
//	if(m_bRButtonFirstPressed && m_bLand) {
	if( zStatus.m_bRButtonFirstPressed && ZIsActionKeyPressed(ZACTION_USE_WEAPON2) && uStatus.m_bLand) {
		...
		case MWT_DAGGER_RARE :
		case MWT_DUAL_DAGGER_RARE :
			zStatus.m_bSkill=true;
			m_fSkillTime.Set_CheckCrc( ZGetGame()->GetTime() );
			uStatus.m_bTumble = false;
			AddDelayedWork(ZGetGame()->GetTime()+0.18f,ZDW_DASH);
			break;
		case MWT_KATANA_RARE :
		...
		case MWT_DAGGER_EXOTIC :
		case MWT_DUAL_DAGGER_EXOTIC :
			zStatus.m_bSkill=true;
			m_fSkillTime.Set_CheckCrc( ZGetGame()->GetTime() );
			uStatus.m_bTumble = false;
			AddDelayedWork(ZGetGame()->GetTime()+0.18f,ZDW_DASH);
			break;
		case MWT_KATANA_EXOTIC :
...
void ZMyCharacter::ProcessGuard()
{
	...
	if(type!=MWT_KATANA && type!=MWT_KATANA_RARE && type!=MWT_KATANA_EXOTIC && type!=MWT_DOUBLE_KATANA && type!=MWT_DOUBLE_KATANA_RARE && type!=MWT_DOUBLE_KATANA_EXOTIC) 
	{
		CHECK_RETURN_CALLSTACK(ProcessGuard);
		return; 
	}
...
#define GUARD_TIME	0.1f
	...
	if(bBothPressed)
	{
		...
		{
				if(strstr(ZGetGameClient()->GetStageName(), "[button05]"))
			{
				if(zStatus.m_bShot==true || zStatus.m_bJumpShot == true || zStatus.m_bSkill == true) return;
			}
...
void ZMyCharacter::WallJump2()
{
	...
	if(strstr(ZGetGameClient()->GetStageName(), "[button02]"))
	{
...
float ZMyCharacter::GetControllabilityFactor()
{
...

	{
		...
		case MWT_MACHINEGUN:
			...
			break;
		case MWT_MACHINEGUN_RARE:
			{
				return CA_FACTOR_MACHINEGUN;
			}
			break;
		case MWT_MACHINEGUN_EXOTIC:
			{
				return CA_FACTOR_MACHINEGUN;
			}
			break;
		case MWT_RIFLE:
		case MWT_RIFLE_RARE:
		case MWT_RIFLE_EXOTIC:
		case MWT_SNIFER:
			{
				return CA_FACTOR_RIFLE;
			}
			break;
		case MWT_SNIFER_RARE:
			{
				return CA_FACTOR_RIFLE;
			}
			break;
		case MWT_SNIFER_EXOTIC:
			{
				return CA_FACTOR_RIFLE;
			}
			break;
		...
		case MWT_ROCKET:
			...
			break;
		case MWT_ROCKET_RARE:
			{
				return CA_FACTOR_ROCKET;
			}
			break;
		case MWT_ROCKET_EXOTIC:
			{
				return CA_FACTOR_ROCKET;
			}
			break;
...
//		if(m_bShot) 
	...
	case ZDW_UPPERCUT :
		if(zStatus.m_bSkill) {	// 아직 상태를 벗어나지 않았다면,,
			...
			if(type == MWT_KATANA || MWT_KATANA_RARE || MWT_KATANA_EXOTIC || type == MWT_DOUBLE_KATANA || type == MWT_DOUBLE_KATANA_RARE || type == MWT_DOUBLE_KATANA_EXOTIC ) {
				ZPostSkill( /*g_pGame->GetTime(),*/ ZC_SKILL_UPPERCUT , sel_type );
			}
		}
		break;
	case ZDW_DASH :
		if(zStatus.m_bSkill) {	// 아직 상태를 벗어나지 않았다면,,
			...
			else if(type == MWT_DAGGER_RARE) {//한손대거..
				ZPostSkill( /*g_pGame->GetTime(),*/ ZC_SKILL_DASH , sel_type );
			}
			else if(type == MWT_DUAL_DAGGER) {//양손대거..
				ZPostSkill( /*g_pGame->GetTime(),*/ ZC_SKILL_DASH , sel_type );
...
{
	if(strstr(strlwr((char*)ZGetGameClient()->GetStageName()), "[button03]"))
	{
...
{
	if(strstr(strlwr((char*)ZGetGameClient()->GetStageName()), "[button04]"))
	{
	/*ZMyCharaterStatusBitPacking & zStatus = m_statusFlags.Ref();
