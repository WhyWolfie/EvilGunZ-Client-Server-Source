			...
			if(CheckRange(pTarget->GetPosition(),pTarget) && m_pDesc->CheckResist(pTarget,&fDamage)) {
				if(ZGetGame()->CanAttack(m_pOwner,pTarget)) {
					if(fDamage)
						...
						pTarget->OnDamaged(m_pOwner,m_TargetPos,ZD_MELEE,MWT_KATANA_RARE,fDamage,m_pDesc->nModCriticalRate*.01f);
						pTarget->OnDamaged(m_pOwner,m_TargetPos,ZD_MELEE,MWT_KATANA_EXOTIC,fDamage,m_pDesc->nModCriticalRate*.01f);
				}
			}
		}
	}
			...
			if ( CheckRange( pTargetObject->GetPosition(), pObject) && m_pDesc->CheckResist( pObject, &fDamage))
			{
				...
				if ( ZGetGame()->CanAttack( m_pOwner, pObject))
				{
					if ( fDamage && (uidTarget == pObject->GetUID()))
						...
						pObject->OnDamaged( m_pOwner, m_TargetPos, ZD_MELEE, MWT_KATANA_RARE, fDamage, m_pDesc->nModCriticalRate*.01f);
						pObject->OnDamaged( m_pOwner, m_TargetPos, ZD_MELEE, MWT_KATANA_EXOTIC, fDamage, m_pDesc->nModCriticalRate*.01f);
				}
				else
				{
					if ( m_pDesc->nModHeal)
