...
//	wsprintf(szText, "당신은 채널 '%s'에 입장하셨습니다.", szChannelName);
	...
	ZChatOutput( "^2Click on either button to the left to view Nightfall or Quest combination information." );
	ZChatOutput( "^2You will recieve an EvilTreasure Box every Tuesday." );
	ZChatOutput( "^2Checkout the EvilShop for LIMITED TIME Items." );
...
//	gid = MMUG_DEVELOPER;
	...
	if(bSpUser)	// 특수유저
	{
        ...
        {
        case 255 : //admin rank
        wsprintf(szText, "[OWNER] ^a%s : %s", szName , szChat);
            break; 
        case 254: //super gm
        wsprintf(szText, "[ADMIN] ^f%s : %s", szName , szChat);
            break;
		case 252: //gm
        wsprintf(szText, "[STAFF] ^c%s : %s", szName , szChat);
            break;
        case 10: //vip
        wsprintf(szText, "[VIP] ^m%s : %s", szName , szChat);
            break;
        case 5: //muted
        wsprintf(szText, "^x%s : %s", szName , szChat);
            break; 
        }
...
{    
        if(strstr(strlwr((char*)ZGetGameClient()->GetStageName()), "[button06]"))
        {
            char szText[ 100];
            MTD_CharInfo emptyInfo;
            const MTD_CharInfo* pCharInfo[1] = {&emptyInfo};
            ZCharacter* pChar1 = ZGetCharacterManager()->Find(ZGetMyUID());
...
//	gid = MMUG_DEVELOPER;
	...
	if(bSpUser)	// 특수유저
	{
        ...
        {
        case 255 :
        wsprintf(szText, "[OWNER] ^a%s : %s", pObjCache->GetName() , szChat);
            break; 
        case 254:
        wsprintf(szText, "[ADMIN] ^f%s : %s", pObjCache->GetName() , szChat);
            break;
		case 252:
        wsprintf(szText, "[STAFF] ^c%s : %s", pObjCache->GetName() , szChat);
            break;
        case 10:
        wsprintf(szText, "[VIP] ^m%s : %s", pObjCache->GetName() , szChat);
            break;
        case 5:
        wsprintf(szText, "^x%s : %s", pObjCache->GetName() , szChat);
            break;
        }
...
void ZGameClient::OnAgentError(int nError)
{
	if (ZGetGame()) {
		...
		ZChatOutput(ChatColor, "Mr Agent Error is here, report it on the EvilGamerZ forums!");
	}
}
...
void ZGameClient::OnGameLevelUp(const MUID& uidChar)
{
	if (ZGetGame())
	{
		...
		if (pCharacter) {
			...
			ZGetSoundEngine()->PlaySound( "nar/levelup", pCharacter->GetPosition()); //Levelup sound 
		}
	}
}
...
void ZGameClient::OnBroadcastDuelRenewVictories(const char* pszChampionName, const char* pszChannelName, int nRoomno, int nVictories)
{
	...
	ZGetSoundEngine()->PlaySound("fx2/stopduel");
...
void ZGameClient::ChangeQuestStage()
{
		...
		ZGetGameClient()->AmIStageMaster() )
	{
		...
		StageSetting.bObserverEnabled = true;
