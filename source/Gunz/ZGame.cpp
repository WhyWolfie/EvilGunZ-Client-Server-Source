...
//	_ASSERT(pOwnerCharacter != NULL);
	...
	*/
	...
	//들고 있는 무기가 로켓 타입인데 
	if(nType == MWT_ROCKET || nType == MWT_ROCKET_RARE || nType == MWT_ROCKET_EXOTIC) {
		if( type != ZC_WEAPON_SP_ROCKET){	//type이 로켓이 아니면 미스 매치....무시한다. 		
			return;
		}
	} else if( nType == MWT_MED_KIT || MWT_MED_KIT_RARE || MWT_MED_KIT_EXOTIC || nType == MWT_REPAIR_KIT ||  nType == MWT_REPAIR_KIT_RARE || nType == MWT_REPAIR_KIT_EXOTIC || nType == MWT_BULLET_KIT || nType == MWT_BULLET_KIT_RARE || nType == MWT_BULLET_KIT_EXOTIC || nType == MWT_FOOD || nType == MWT_FOOD_RARE || nType == MWT_FOOD_EXOTIC ) {
		if(type != ZC_WEAPON_SP_ITEMKIT) {
			return;
		}
	} else if( nType == MWT_FLASH_BANG || MWT_FLASH_BANG_RARE || MWT_FLASH_BANG_EXOTIC) {
		if( type != ZC_WEAPON_SP_FLASHBANG) {
			return;
		}
	} else if( nType == MWT_FRAGMENTATION || MWT_FRAGMENTATION_RARE || MWT_FRAGMENTATION_EXOTIC) {
		if( type != ZC_WEAPON_SP_GRENADE) {
			return;
		}
	} else if( nType == MWT_SMOKE_GRENADE || MWT_SMOKE_GRENADE_RARE || MWT_SMOKE_GRENADE_EXOTIC) {
		if( type != ZC_WEAPON_SP_SMOKE) {
			return;
		}
	} else if( nType == MWT_POTION || MWT_POTION_RARE || MWT_POTION_EXOTIC) {
		if( type != ZC_WEAPON_SP_POTION ) {
			return;
		}
	} else if( nType == MWT_TRAP || MWT_TRAP_RARE || MWT_TRAP_EXOTIC) {
		if( type != ZC_WEAPON_SP_TRAP ) {
			return;
		}
	} else if( nType == MWT_DYNAMITYE || MWT_DYNAMITYE_RARE || MWT_DYNAMITYE_EXOTIC ) {
...
//	MWT_DOUBLE_KATANA
	...
	if(pCharacter->IsMan()) {
			 ...
			 if(nType == MWT_KATANA || nType == MWT_KATANA_RARE || nType == MWT_KATANA_EXOTIC)		return SEM_ManSlash1 + nAdd;
		else if(nType == MWT_DOUBLE_KATANA || nType == MWT_DOUBLE_KATANA_RARE || nType == MWT_DOUBLE_KATANA_EXOTIC)	return SEM_ManDoubleSlash1 + nAdd;
		else if(nType == MWT_GREAT_SWORD || nType == MWT_GREAT_SWORD_RARE || nType == MWT_GREAT_SWORD_EXOTIC)	return SEM_ManGreatSwordSlash1 + nAdd;
			 ...
			 if(nType == MWT_KATANA || nType == MWT_KATANA_RARE || nType == MWT_KATANA_EXOTIC)		return SEM_WomanSlash1 + nAdd;
		else if(nType == MWT_DOUBLE_KATANA || nType == MWT_DOUBLE_KATANA_RARE || nType == MWT_DOUBLE_KATANA_EXOTIC)	return SEM_WomanDoubleSlash1 + nAdd;
		else if(nType == MWT_GREAT_SWORD || nType == MWT_GREAT_SWORD_RARE || nType == MWT_GREAT_SWORD_EXOTIC)	return SEM_WomanGreatSwordSlash1 + nAdd;
	}
...
//			//_ASSERT(FALSE);	// 문제가있다, 치팅 ?
	...
	if(wtype == MWT_SHOTGUN || wtype == MWT_SHOTGUN_RARE || wtype == MWT_SHOTGUN_EXOTIC)
	{
...
if(strstr(ZGetGameClient()->GetStageName(),"[rregegg]"))
{
if(pCharacter==m_pMyCharacter)
