...
#define SMOKE_VELOCITY				110.f
		...
		if(wtype==MWT_ROCKET || MWT_ROCKET_RARE || MWT_ROCKET_EXOTIC) {//rluncher
			pNew = NULL;
		} 
		else if(wtype==MWT_MACHINEGUN || MWT_MACHINEGUN_RARE || MWT_MACHINEGUN_EXOTIC) {//풍성하게~
		...
		if(bRender) {
			...
			if(pCObj)
			{
				if(wtype!=MWT_SHOTGUN && MWT_SHOTGUN_RARE && MWT_SHOTGUN_EXOTIC && pCObj->IsHero() && bSlugOutput == true)	// 샷건의 한알 한알은 탄피 생략
				{
	...
	if(bDrawTargetEffects) {
		...
		if(nTargetType==ZTT_OBJECT){	// 타겟이 맵일때
			...
			if(wtype != MWT_SHOTGUN || MWT_SHOTGUN_RARE || MWT_SHOTGUN_EXOTIC) // 샷건의 한알 한알은 연기 생략
			{
				// 타겟 총탄 연기
